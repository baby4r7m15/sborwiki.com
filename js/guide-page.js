(async function () {
  ...
  function wrapSection(id, chip, title, innerHtml, searchableText = "") {
    return `
      <section
        class="hud-panel guide-section-anchor guide-section-card"
        id="${escapeHtml(id)}"
  ...
  function renderText(block, id) {
    return wrapSection(
      ...
      "TEXT",
      block.title || "SECTION",
      `<div class="hud-lore">${renderParagraphs(block.paragraphs || [])}</div>`,
    ...
    return wrapSection(
      ...
      `
        <img
          ...
          alt="${escapeHtml(block.alt || block.title || "Guide image")}"
          loading="lazy"
        />
  ...
  function renderSteps(block, id) {
    return wrapSection(
      ...
      "GUIDE",
      block.title || "STEPS",
      `<div class="hud-lore">${renderList(block.items || [], true)}</div>`,
      buildSearchText(block)
    );
  }
  ...
  function renderBulletList(block, id) {
    return wrapSection(
      ...
      "INFO",
      block.title || "LIST",
      `<div class="hud-lore">${renderList(block.items || [], false)}</div>`,
      buildSearchText(block)
    );
  }
  ...
  function renderSection(block, index) {
    ...
    switch (block.type) {
      case "text":
        return renderText(block, id);
      case "image":
        return renderImage(block, id);
      case "steps":
        return renderSteps(block, id);
      case "list":
        return renderBulletList(block, id);
      case "table":
        return renderTable(block, id);
      case "callout":
        return renderCallout(block, id);
      default:
        return wrapSection(
          id,
          "ERROR",
          "UNKNOWN BLOCK",
          `<div class="hud-lore"><p>Unsupported block type: <strong>${escapeHtml(block.type || "undefined")}</strong></p></div>`,
          buildSearchText(block)
        );
    }
  }
  ...
  function renderGuideMeta(guide) {
    ...
    if (Array.isArray(guide.intro)) {
      bits.push(renderParagraphs(guide.intro));
    }

    if (guide.updated) {
      bits.push(`
        <div class="guide-meta-row">
          <span class="guide-badge">Updated: ${escapeHtml(guide.updated)}</span>
        </div>
      `);
    }
    ...
    if (Array.isArray(guide.tags) && guide.tags.length) {
      bits.push(`
        <div class="guide-meta-row">
          ${guide.tags.map(tag => `<span class="guide-badge">${escapeHtml(tag)}</span>`).join("")}
        </div>
      `);
    }
  ...
  function buildToc() {
    ...
    const items = renderedSectionCards.map(card => ({
      id: card.id,
      title: card.dataset.title || "Section"
    }));

    tocEl.innerHTML = items
      .map(item => `<li><a href="#${escapeHtml(item.id)}" data-target="${escapeHtml(item.id)}">${escapeHtml(item.title)}</a></li>`)
      .join("");
  ...
  function filterSections(query) {
    ...
    const existingNoResults = document.getElementById("guideNoResults");
    ...
    if (!visibleCount) {
      sectionsEl.insertAdjacentHTML(
        "beforeend",
        `
          <section class="hud-panel" id="guideNoResults">
            <div class="panel-head">
              <span>NO RESULTS</span>
              <span class="panel-chip">SEARCH</span>
            </div>
            <div class="hud-lore">
              <p>No guide sections matched your search.</p>
            </div>
          </section>
        `
      );
  ...
  try {
    ...
    if (guide.bannerImage) {
      bannerEl.src = guide.bannerImage;
    }
